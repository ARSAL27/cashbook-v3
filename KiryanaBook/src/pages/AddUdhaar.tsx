...
export const AddUdhaar: React.FC = () => {
  ...
  const bgGradientType = type === 'diye' ? 'from-emerald-500 to-teal-700' : 'from-rose-500 to-red-700';
  const glowColorType = type === 'diye' ? 'shadow-emerald-500/50' : 'shadow-rose-500/50';

  return (
    <PageTransition>
      <div className={`w-full min-h-screen pb-[350px] transition-colors duration-500 font-outfit max-w-md mx-auto ${isDarkMode ? 'bg-[#050505]' : 'bg-[#FAFAFA]'}`}>
        ...
        >
            ...
            <div className="relative space-y-1 text-center">
                <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-white/70 text-[10px] font-black uppercase tracking-[0.3em]">
                   {type === 'diye' ? '💸 Maine Diye (Udhaar / Credit)' : '💰 Maine Liye (Wapsi / Debit)'}
                </motion.p>
        ...
        <div className="px-6 -mt-6 relative z-20">
            <div className="flex bg-white dark:bg-[#141414] p-1.5 rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] dark:shadow-none border border-gray-100 dark:border-white/10 backdrop-blur-xl">
                <button 
                  ...
                  className={`flex-1 py-3.5 rounded-xl font-black text-[12px] uppercase tracking-[0.15em] transition-all duration-300 ${type === 'diye' ? 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}
                >
                  Maine Diye
                </button>
                <button 
                  onClick={() => { setType('liye'); triggerHaptic(); }}
                  className={`flex-1 py-3.5 rounded-xl font-black text-[12px] uppercase tracking-[0.15em] transition-all duration-300 ${type === 'liye' ? 'bg-rose-50 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 shadow-sm' : 'text-gray-400 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5'}`}
                >
                  Maine Liye
                </button>
            </div>
                          ...
                          <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col">
                                ...
                                <span className="text-[14px] font-black text-blue-800 dark:text-blue-300">
                                    {newBalance >= 0 ? (isSupplier ? 'Unko advance de diya' : 'Unse lene hain') : (isSupplier ? 'Unke paas due hain' : 'Unka advance hai')}
                                </span>
                              </div>
                          </div>
        ...
        <div className="fixed bottom-0 inset-x-0 mx-auto max-w-md bg-white dark:bg-[#0A0A0A] rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.08)] dark:shadow-[0_-10px_40px_rgba(0,0,0,0.5)] border-t border-gray-100 dark:border-white/5 pt-6 pb-8 px-6 z-[100] backdrop-blur-2xl">
            {/* Elegant Glassy Numpad */}
            <div className="grid grid-cols-3 gap-y-3 gap-x-4 mb-6">
                {['1','2','3','4','5','6','7','8','9','.', '0'].map((n) => (
                    <button 
                        key={n} 
                        onClick={() => handleNumpad(n)} 
                        className="h-[60px] rounded-[1.2rem] text-[22px] font-black active:scale-90 transition-all text-center bg-gray-50 dark:bg-[#141414] text-gray-800 dark:text-white hover:bg-gray-100 dark:hover:bg-[#1A1A1A] border border-gray-100 dark:border-white/5"
                    >
                        {n}
                    </button>
                ))}
                <button 
                  ...
                  className="h-[60px] rounded-[1.2rem] text-[22px] font-black active:scale-90 transition-all flex items-center justify-center bg-rose-50 dark:bg-rose-500/10 text-rose-500 border border-rose-100 dark:border-rose-500/20"
                >
                    <Delete size={22} strokeWidth={2.5} className="ml-1" />
                </button>
            </div>
            
            <button 
                onClick={handleSave} 
                disabled={!customerName.trim() || amount === '0'}
                className={`w-full group py-4.5 rounded-[1.5rem] active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-3 overflow-hidden relative ${
                    customerName.trim() && amount !== '0' 
                    ? `bg-gradient-to-r ${bgGradientType} ${glowColorType} text-white shadow-xl` 
            ...
            >
                ...
                <CheckCircle2 size={20} strokeWidth={3} className={customerName.trim() && amount !== '0' ? 'text-white' : ''} />
                <span className="text-[15px] font-black tracking-[0.2em] uppercase mt-0.5">
                   ENTRY SAVE
                </span>
            </button>
        </div>
