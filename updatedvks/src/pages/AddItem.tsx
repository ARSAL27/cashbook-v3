...
export const AddItem: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            // Duplicate-barcode check from origin/main — keeps users from creating
            // two stock rows for the same SKU.
            if (existingInStock) {
        ...
        try {
            const base64Images = await Promise.all(files.map(file => {
                return new Promise<string>((resolve, reject) => {
                    ...
                    // Hardened: reject on read failures so the outer toast surfaces real errors.
